...
#Define perturbation step_size \eps and #steps
eps = 0.1
steps = 10
...
H0, pcomponents = create_TFIM(n_qubits = 4, g = 0)

#Solve analytically for g = 0

H_now = None
eigen_vector_now = None
X_param_now = 
ZZ_param_now = 
...
#For total steps = 0 --> 10, go from g = 0.1 to g = 1.1
for r in range(steps):
    #Loop 10 times, each time restarting the problem with outputs from previous step.
    H_int, components_int = create_TFIM(n_qubits = n_qubits, g = eps*(r+1))
    #Call Gradient Descent
    h_theta, h_count, h_eigen_value, h_eigen_vector, h_grad = hn_grad_desc_quantum(H = H_int, components = components_int, 
                                                                          X_param_set = X_param_now, ZZ_param_set = ZZ_param_now, 
                                                                          circuit_input=circuit_input,
                                                                          MAXITERS=1000, eta = 0.001,  GRADTOL = 0.00001,
                                                                          n_qubits = n_qubits, layers = layers, plotting = 'on', logging='on')

    #Store Statistics
    iter_counter.append(h_count)
    iter_eigen.append(h_eigen_value)
    iter_theta.append(h_theta)
    
    #Update the ground state before next perturbation.
    X_param_now = h_theta[1]
    ZZ_param_now = h_theta[0]
    eigen_vector_now = h_eigen_vector
    H_now = H_int
    
#Minitor Final convergence

    #Diagonalize
e, v = LA.eigh(H_now)

    #Print min eigen value
print('Calculated Min Eigen = ', iter_eigen[-1])

    #Print Error
print('Absolute Error in min eigen value calculation = ', np.abs(iter_eigen[-1] - np.min(e)))

    #Calculate Overlap
overlap_calculator(min_gd = eigen_vector_now, eigen_values = e, eigen_vectors = v)

#Plot performance - time/#iterations
plt.plot(range(steps),iter_counter)
plt.title('Track Steps required')
